...
Per case it produces 2 NESTED solid bodies (no boolean union):
  1. propeller   (imported from geometry/<case>/<case>.step)
  2. outer box   +/-180 mm (X,Y), Z = prop_centre +/- 300 mm    -> static fluid

Autodesk CFD treats the nesting as: propeller = solid, box-minus-propeller =
air. Rotation is applied to the PROPELLER solid (CFD spins the surrounding air
automatically), so NO separate rotating-fluid cylinder is needed -- that was an
OpenFOAM-MRF holdover. If finer mesh near the rotor is wanted, add a cylindrical
mesh-refinement REGION inside CFD (not a geometry body). Identical domain for
A/B/C except the axial position, which follows each propeller's centre.
...
def build(app, case):
    ...
    # outer box (separate body, encloses the propeller)
    ...
    print("  bodies now=%d (prop + box)" % nb)
