...
- 対象：**プロペラ本体（最内ソリッド）**。中間の円筒流体は**メッシュ細分化用**として静止空気のまま残す（§6）。
...
- 自動メッシュ ＋ **回転円筒の体積細分化**（局所メッシュ 0.25 cm）。
...
- 乱流モデル：**SST k-ω を推奨**（既定の k-ε のままにしない）。Re≈500 と低く薄翼で剥離が支配的なため、
  壁まで積分する SST k-ω が適する（k-ε は壁関数前提で粘性低層を解かず低 Re・剥離に弱い）。
  さらに Re≈500 は遷移以下なので **層流（Laminar）でも再計算**して優劣順を確認（A/B/C すべて）。
...
4) §3 材料 Air → §5 BC（外周 0 Pa）→ §4 回転運動 10.472 rad/s（=100rpm）+Z
5) §6 メッシュ（外箱 0.5・ボックス 1cm・シリンダー 0.25cm）→ §7 Solve（非定常 dt0.006/2.4s/400step, 乱流, 2ⁿ コア）
