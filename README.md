...
> [reports/10_autodesk_cfd.md](reports/10_autodesk_cfd.md)，3 ケース統一の CFD 設定・操作・自動化を
> [reports/11_autodesk_cfd_gui.md](reports/11_autodesk_cfd_gui.md)（単一の手順書）に記す．
...
- **CFD（最終）**：**Autodesk CFD 2027** で **A/B/C を完全に同一設定**で評価する
  （[reports/11_autodesk_cfd_gui.md](reports/11_autodesk_cfd_gui.md)）．座標系は全工程で **Z 軸 = 回転軸 = 推力方向**．
...
scripts/
  ...
  cfd_domain_inventor.py ★ CFD 用ドメイン（プロペラ＋外箱の2ボリューム）STEP を生成
...
実ソルブは Autodesk CFD 2027（GUI）で行う．**設定・操作・結果抽出・自動化を集約した
単一の手順書は [reports/11_autodesk_cfd_gui.md](reports/11_autodesk_cfd_gui.md)**．要約：

本問題は **回転翼の非定常解析（Transient）**である（授業資料 `Exercise05.pdf` 準拠）．

1. **取り込み**：`geometry\<case>\<case>_cfd.step` を新規スタディに取り込む（入れ子2ボリューム：
   内側＝プロペラ，外側＝外箱．**回転円筒は廃止**＝回転はプロペラに与えるため不要）．
2. **材料**：プロペラ＝固体，外箱＝**空気**．
...
4. **モーション**：**プロペラ本体（最内ソリッド）をボリューム選択**し **回転運動，軸 +Z（0,0,1），
   ...
   回転は**流体ではなくプロペラに与える**（CFD が周囲流体を非定常で回す）．これで回転円筒は不要．
5. **メッシュ**：自動．薄翼は **☑サーフェス細分割**＋「サイズ調整」スライダーで
   **予想要素数 ≈ 数百万（1〜5M）**に追い込む（cm 入力不要．例：サイズ調整 0.4＋サーフェス細分割で
   約 92 万）．粗すぎは翼が解けず NG，数千万は RAM 不足で NG．近傍を細かくしたい場合のみ任意で
   シリンダー細分化“領域”（0.25 cm，Y 角度 90°）．**3ケース同一**設定．
6. **実行（非定常）**：解析モード＝**非定常解析**，**時間刻み 0.003 s（1.8°/step）・実行ステップ 400
   （＝2 回転．安定テストは 200＝1 回転）・内部反復 3・保存 0.03 s**．**終了時刻＝−1，次から継続＝t0，
   インテリジェント解析制御＝無効**（この 4 つが揃わないと step 0 で即「正常終了」する）．
   ソリューションコントロールで**圧縮性流体（全温度≈300 K，伝熱オフ）**＋**層流**（回転翼は dt を細かく
   ＋圧縮性にすると安定．SST k-ω は発散しやすいので安定確認後に追加）．
7. **結果（CSV）**：壁面計算機は使えない．`…\設計1\シナリオ1\シナリオ1_*****.csv` の
   **Hydraulic ForceZ（dyne）**を最終1回転で平均 →×10⁻⁵ で **推力 \(|F_z|\)** ［N］．
   TorqueZ（dyne-cm）×10⁻⁷ で **トルク \(M_z\)** ［N·m］．
8. **繰り返し**：B/C も同じ設定（変えるのは形状のみ）．層流で完走後，**SST k-ω でも再計算**して
   A→B→C の優劣順が乱流モデルに依らないことを確認．

> 回転軸は Z なので**回転中心・トルク軸点は 3 ケースとも (0,0,0)** で良い．自動化は形状取り込みまで
> API で確認済み（[scripts/cfd_run.py](scripts/cfd_run.py)）だが，材料以降は enum 非公開のため GUI 推奨．

解析完了後の**数値・画像の取得手順**は
[reports/12_autodesk_cfd_results.md](reports/12_autodesk_cfd_results.md)（CSV からの \(|F_z|\) 平均，
圧力コンタ・等圧面の書き出し，命名規約，レポートへの反映）．
