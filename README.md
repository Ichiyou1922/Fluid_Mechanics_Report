...
5. **メッシュ**：自動→外箱 0.5→ボックス細分化 1 cm・**シリンダー細分化 0.25 cm（Y 角度 90°）**．
   薄翼に 3 要素目安（3ケース同一）．
6. **実行（非定常）**：解析モード＝**非定常解析**，**時間刻み 0.006 s・終了 2.4 s（4回転）・
   実行ステップ 400・内部反復 1・保存 0.03 s**．ソリューションコントロールで**非圧縮性**，乱流モデルは
   **SST k-ω**（低 Re・薄翼の剥離向け．既定の k-ε は不可）．インテリジェント解析制御は**無効**．
   （実行ステップが 1〜0 だと一瞬で終わるので必ず確認）
