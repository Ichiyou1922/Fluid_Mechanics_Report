...
3. **モーション**：中間の円筒流体に **回転運動 100 rpm，軸 +Z，中心 (0,0,0)**．
4. **境界条件**：外箱の外側6面に **圧力 0 Pa（ゲージ圧）**．壁は自動 no-slip．
5. **メッシュ**：自動＋翼面サーフェス細分割（薄翼に3要素目安．3ケース同一）．
6. **実行**：非圧縮・**SST k-omega**・定常・伝熱オフ・反復は \(F_z\) 平坦化まで・コア 2ⁿ．
7. **結果**：「結果」タブ → **壁面計算機**でプロペラ壁を選択 → **力 Z＝推力 \(|F_z|\)**，
   モーメント Z＝トルク．
